...
<always-applicable>
- 改库前先读 [`.agents/skills/kk-request/rules/boundaries.md`](.agents/skills/kk-request/rules/boundaries.md)：做什么 / 不做什么 + 依赖环境硬约束。
...
1. **改 kk-request 库本身**（hook / 401 刷新 / 响应解析 / 错误模型 / SSE / 架构）：
   读 [`.agents/skills/kk-request/SKILL.md`](.agents/skills/kk-request/SKILL.md)，按其 **Common Tasks** 表路由到对应 rules / workflows / references。
...
- **新任务开始** → 重读 [`.agents/skills/kk-request/SKILL.md`](.agents/skills/kk-request/SKILL.md) 的 Common Tasks。
- **写 / 改 hook 前** → 读 [`.agents/skills/kk-request/rules/hook-authoring.md`](.agents/skills/kk-request/rules/hook-authoring.md)。
- **改错误处理前** → 读 [`.agents/skills/kk-request/rules/error-model.md`](.agents/skills/kk-request/rules/error-model.md)。
