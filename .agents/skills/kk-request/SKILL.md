...
| 改 schema 校验 / 响应数据校验 | [rules/error-model.md](rules/error-model.md) + [references/architecture.md](references/architecture.md)（Schema 校验一节） | `src/client/request.ts`、`src/types/options.ts` |
...
## Known Gotchas（4 条最关键不变量，详见 [references/gotchas.md](references/gotchas.md)）
...
4. **schema 校验在 `request()` 末尾、业务码之后**；`SchemaValidationError` 不走 `onError`/`makeErrorMessage`（结构层错误，详见 [rules/error-model.md](rules/error-model.md)）。
